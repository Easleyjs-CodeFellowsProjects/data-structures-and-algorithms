...
const count = (target, input) => {
  let totalCount = 0;
  const counts = input.map(numArr => { return numArr.reduce((acc, curr) => curr == target ? acc + 1 : acc ,totalCount);
  });
  return counts.reduce((acc, curr) => acc + curr, totalCount);
};
...
const totalSum = (input) => {
  let totalCount = 0;
  return input.reduce((acc, curr) => acc + curr.reduce(
    (acc, curr) => acc + curr, totalCount), totalCount);
};
...
const divisibleByFiveTwoToThePower = (input) => {
    return input.map(arr => { if( arr.length === 0) {
          return arr
      } else {        
          return arr.filter( newArr => (newArr % 5 === 0) && typeof newArr === 'number' )
                    .map( filteredArr => 2 ** filteredArr )
      }
  })
};
...
let findShortest = (data) => {
  return data[data.map( (person, idx) => Number(person.height) )
    .reduce((index, value, i, array) => value < array[index] ? i : index, 0)]
    .name
    // reduce from https://linuxhint.com/get-the-index-of-the-min-value-in-an-array-in-javascript/
