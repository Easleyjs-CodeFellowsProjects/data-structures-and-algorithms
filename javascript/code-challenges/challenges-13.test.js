...
of phone number strings in (XXX) XXX-XXXX format
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(telNum => {
    return telNum.substring(1,4) + telNum.substring(6,9) + telNum.substring(10,14);
  });
};
...
const onlyOddChars = (str) => {
  return str.split('').map( (letter, idx) => { if (idx % 2 !== 0) return str.charAt(idx); } ).join('');
};
...
const allHappy = (arr) => {
  return arr.every(ele => ele.includes(':)'));
};
