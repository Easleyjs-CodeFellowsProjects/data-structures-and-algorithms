...
const sortStarWarsCharacters = (starWarsArr) => {
  return starWarsArr.sort((a, b) => {
    let aHeight = Number(a.height)
    let bHeight = Number(b.height)
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.map(item => {
    let itemDesc = item.slice(item.indexOf(' ')+1,item.length).trim()
    result.push(itemDesc.slice(itemDesc.indexOf(' '),itemDesc.length).trim())
  })
  return result;
};
