...
const raisedToTheThird = (arr) => {
  const thirdRaisedArr = []
  arr.forEach(element => {
    thirdRaisedArr.push( Math.pow( element, 3 ) )
  });
  return thirdRaisedArr
};
...
const forEachTwoToThe = (arr) => {
  const returnArr = []
  arr.forEach(number => {
    returnArr.push( Math.pow(2, number) )
  })
  return returnArr
};
