...
Write a function named screenForNames that takes in an array of strings and uses Regex
to create a new array of only those strings that match the following rules:
...
const screenForNames = (arr) => {
  return arr.filter(name => /^(Mr|Mrs|Ms|Dr)\.\s?[A-Za-z]+$/g.test(name));
};
...
let biggerThanLuke = (arr) => {
  return arr.filter(person => Number(person.mass) > 77)
    .map(character => {
...
const sortBy = (property, arr) => {
  return arr.sort((a, b) => {
    const aValue = Number.parseInt(a[property]) ? a[property] : a[property].charAt(0);
    const bValue = Number.parseInt(b[property]) ? b[property] : b[property].charAt(0);
    if (aValue > bValue) {
      return 1;
    }
    if (aValue < bValue) {
...
Your function should check to see if any row, column, or either diagonal direction
has three matching "X" or "O" symbols (non-empty strings), three-in-a-line.
...
const detectTicTacToeWin = (board) => {
  ...
  const checkLine = (a, b, c) => {
    return a + b + c === 'XXX' || a + b + c === 'OOO' ? true : false;
  };
  if ( checkLine(board[0][0], board[1][1], board[2][2]) || checkLine(board[2][0], board[1][1], board[0][2]) ) {
    gameWon = true;
  } else {
    for (let i=0;i<3;i++) {
