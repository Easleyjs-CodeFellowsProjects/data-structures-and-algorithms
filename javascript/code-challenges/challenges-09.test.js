...
const maxInArray = (arr) => {
  const result = arr.reduce((accumulator, currentValue) => accumulator < currentValue ? currentValue : accumulator,0);
  return result;
};
...
const getHouses = (arr) => {
  //let houses = [];
  return arr.map(character => { return character.house; });
  //return houses;
};
...
const hasChildrenValues = (arr, character) => {
  let hasKids = false;
  arr.map(char => {
    if (char.name === character && char.children) hasKids = true;
  });
  return hasKids;
};
