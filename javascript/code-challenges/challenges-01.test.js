...
const addExclamation = (arr) => {
  const stringsPlusBang = [];
  arr.forEach(string => {
    stringsPlusBang.push(string + "!")
  });
  return stringsPlusBang
};
...
const allUpperCase = (arr) => {
  const toUpperArr = [];
  arr.forEach(string => {
    toUpperArr.push(string.toUpperCase());
  });
  return toUpperArr;
};
...
const speaker = (words, callback) => {
  const upperBangWordsArr = [];
  words.forEach(word => {
    upperBangWordsArr.push(callback(word))
  });
  return upperBangWordsArr
};
...
const createList = (availableItems) => {
  ...
  availableItems.forEach(item => {
    if (item.available) {
      availableItemsArr.push(item.name)
    }
  });
